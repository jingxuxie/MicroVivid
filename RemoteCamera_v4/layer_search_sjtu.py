...
def calculate_local_contrast(img_raw, cnt_large, sample_value, bk_color_y):
    ...
    hist_cut_start = int(round(bk_color_y -5))
...
def layer_search_sjtu(filename, thickness = '285nm', contrast_range = [0.04, 0.2]):
...
#    plt.plot(hist_y)
    ...
    lowerBound = int(bk_color_y + 3)
    upperBound = int(bk_color_y + 20)
    cv2.inRange(img, lowerBound, upperBound, mask)
...
#            cv2.rectangle(img_raw_draw,(x,y),(x+w,y+h),(0,255,0),2)
            cv2.rectangle(img_incr_contr,(start_2,start_1),(end_2,end_1),(0,255,0),2)
            
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(img_incr_contr,str(round(contrast[k],3)),(int(x),int(y)), font, 1,(255,255,255),2,cv2.LINE_AA)
            print(contrast[k])
            isLayer = True
            k += 1
...
#    cv2.imshow('4', img_incr_contr)
    return isLayer, img_incr_contr
...
#                print(input_name)
        ...
        ret, img_out = layer_search_sjtu(input_name, thickness)
...
#    layer_search_sjtu('F:/2020/8/7/08-22-2020-13.png', thickness='285nm')
#    test_run('285nm')
    
    #img0 = cv2.resize(img0, (1200,800))
    
    img_test = cv2.imread('F:/2020/8/7/08-22-2020-13.png')
    x_middle = int(img_test.shape[1]/2)
    y_middle = int(img_test.shape[0]/2)
    img_1 = img_test[:y_middle, :x_middle, :]
    img_2 = img_test[:y_middle, x_middle:, :]
    img_3 = img_test[y_middle:, :x_middle, :]
    img_4 = img_test[y_middle:, x_middle:, :]
    layer_search_sjtu(img_3, thickness='285nm')
