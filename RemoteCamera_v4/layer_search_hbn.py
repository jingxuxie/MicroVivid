# -*- coding: utf-8 -*-
"""
Created on Mon Aug  3 15:35:43 2020
...
def add(img0, value):
    img = img0.copy()
    img_out = np.zeros((img.shape[0], img.shape[1]), dtype = np.uint8)
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            ...
            if temp < 0:
                ...
                temp = 255
            img_out[i,j] = round(temp)
    return img_out

@jit
def multiply(img0, value):
    img = img0.copy()
    img_out = np.zeros((img.shape[0], img.shape[1]), dtype = np.uint8)
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
...
#                i -= 1
#            else:
            local_contrast_y = calculate_local_contrast(img_raw, cnt_large, \
                                                      mean_val_y, bk_color[0], 'y')
            local_contrast_b = calculate_local_contrast(img_raw, cnt_large, \
                                          mean_val_b, bk_color[1], 'b')
            local_contrast_g = calculate_local_contrast(img_raw, cnt_large, \
                                          mean_val_g, bk_color[2], 'g')
            local_contrast_r = calculate_local_contrast(img_raw, cnt_large, \
                                          mean_val_r, bk_color[3], 'r')
            if local_contrast_y > contrast_range[1] or local_contrast_y < contrast_range[0]:
                contour_segmentation.pop(i)
                i -= 1
            else:
                contrast_segmentation.append(local_contrast_y)
            i += 1
            #print(mean_val)
...
def calculate_local_contrast(img_raw, cnt_large, sample_value, bk_color, channel = 'y'):
    #use gray scale
    ...
    img_b, img_g, img_r = cv2.split(img_rec)
    img_y = cv2.cvtColor(img_rec, cv2.COLOR_BGR2GRAY)
    ...
    hist_cut_start = int(round(bk_color -2))
    hist_cut_end = int(round(sample_value + 1))
    ...
    local_contrast = (sample_value - local_bk)/local_bk
...
def layer_search_hbn(filename, thickness = '285nm', contrast_range = [0.02, 0.10]):
...
#    plt.plot(hist_y)
    bk_color_y = np.argmax(hist_y[100:]) + 100
    ...
    lowerBound = int(bk_color_y + 1)
    upperBound = int(bk_color_y + 10)
    cv2.inRange(img, lowerBound, upperBound, mask)
#    cv2.imshow('mask', mask)
    ...
    shift = bk_color_y-5
    stretch = 255/10
    img_incr_contr = add(img, -shift)
    img_incr_contr = multiply(img_incr_contr, stretch)
#    cv2.imshow('img_incr_contr', img_incr_contr)
    
#    threshold = round(5 * stretch)
    
#    ret,img_binary = cv2.threshold(img_incr_contr, threshold, 255, cv2.THRESH_BINARY)
#    cv2.imshow('img_binary', img_binary)
#    hist_y = cv2.calcHist([img_incr_contr], [0], None, [256], [0,255])
#    plt.plot(hist_y)
...
#    dilation = cv2.dilate(img_open,kernel_dilation,iterations = 1)
...
#            cv2.rectangle(img_raw_draw,(x,y),(x+w,y+h),(0,255,0),2)
            cv2.rectangle(img_raw,(start_2,start_1),(end_2,end_1),(0,255,0),2)
            
            font = cv2.FONT_HERSHEY_SIMPLEX
            cv2.putText(img_raw, str(round(contrast[k],3)),(int(x),int(y)), font, 1,(255,255,255),2,cv2.LINE_AA)
            isLayer = True
            k += 1
...
#                print(input_name)
        ...
        ret, img_out = layer_search_hbn(input_name, thickness)
...
#    img0 = cv2.imread('F:/2020/1/25/01-28-2020-36.jpg')
    layer_search_hbn('F:/2020/7/20/08-03-2020-3.png', thickness='285nm')
#    test_run('285nm')
